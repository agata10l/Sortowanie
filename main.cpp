...
int main()
{
	...
	for (int i = 0; i < 3; i++)//algorytm
	{
		...
		{
			plik << "Tablica rozmiar: " << tablica_rozmiary[j] << endl;
			for (int g = 0; g < 7; g++)//procent posortowania
			{
				plik << "Procent: " << ile_posortowanych_tab[g] << endl;
				czas = 0;
				for (int k = 0; k < 100; k++)// 100-liczba testów
				{
	...
	for (int i = 0; i < 3; i++)//algorytm
	{
		...
		for (int j = 0; j < 5; j++)//rozmiar tablicy
		{
			...
			czas = 0;
			for (int k = 0; k < 100; k++)// 100-liczba testow
			{
				system("cls");
				cout << "Numer algorytmu: " << i << endl;
				cout << "Rozmiar tablicy: " << tablica_rozmiary[j] << endl;
